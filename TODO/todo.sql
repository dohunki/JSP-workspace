# 데모 데이터베이스 생성하고 utf8mb4(모든 문자, 한글 및, 이모지 포함)
...
DROP TABLE IF EXISTS todos; # 할 일 테이블이 있으면 삭제
CREATE TABLE `todos` (
  `id` INT NOT NULL AUTO_INCREMENT,	# 자동으로 1씩 증가 생성됨
  `description` varchar(255) DEFAULT NULL,	# 할 일 설명
  `is_done` bit(1) NOT NULL,				# 완료 됨
  `target_date` datetime(6) DEFAULT NULL,	# 목표 날짜
  `username` varchar(255) DEFAULT NULL,		# 유저 이름
  `title` varchar(255) DEFAULT NULL,		# 할 일 목록
  PRIMARY KEY (`id`)
);
