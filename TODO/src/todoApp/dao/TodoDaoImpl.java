...
// DB연결하고 각 기능에 맞게 작업한다. 데이터베이스 todos 테이블에 CRUD 작업
	...
	public List<Todo> selectAllTodos() {
		...
		try {
			...
			// 결과가 여러 줄일 경우 while() 사용하여 처리, 한 줄일 때 if() 사용
			while(rs.next()) { // 결과가 있을 경우에 값을 저장한다. (없을 때 저장하면 에러발생)
				long id= rs.getLong("id");
				String title= rs.getString("title");
				String username= rs.getString("username");
				String description= rs.getString("description");
	...
	public boolean deleteTodo(long todoId) {
		...
		try {
			...
			rowDeleted = pstmt.executeUpdate() > 0; // 한 줄 이상 삭제가 되면 true
			
		} catch (SQLException e) {
			System.out.println("SQL 삭제에러");
			return false;
		}
