...
//		out.println("DB연결 테스트 완료!");
		...
		} else if(action.equals("createaccount")) { // 가입하기 페이지에서 작성 후 가입하기버튼을 클릭시!
			String email = request.getParameter("email");
			String password = request.getParameter("password");
...
//			request.setAttribute("message", "");
			...
			if(!password.equals(repeatPassword)) {
				...
				if(!user.validate()) {
					...
					request.getRequestDispatcher("/createaccount.jsp").forward(request, response);
				} else { // 유효성 검사 합격했을 경우 email 중복확인후 새 계정 만들기
					try {
						if(account.exists(email)) { // 중복되는 이메일이 있을 경우
		...
		try {
			conn.close(); // 실제로는 conn을 닫는 것 대신에 커넥션을 커넥션 풀로 보냄
		} catch (SQLException e) {
			out.println("DB연결 닫는 중 에러");
		}
	}
